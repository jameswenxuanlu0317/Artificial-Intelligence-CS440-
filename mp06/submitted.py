...
def minimax(side, board, flags, depth):
    ...
    for move in generateMoves(side, board, flags):
        new_side, new_board, new_flags = chess.lib.makeMove(side, board, move[0], move[1], flags, move[2])
        score, next_movelist, next_movetree = minimax(new_side, new_board, new_flags, depth - 1)
...
def alphabeta(side, board, flags, depth, alpha=float('-inf'), beta=float('inf')):
    '''
    ...
    Input:
      side (boolean): True if player1 (Min) plays next, otherwise False
      board (2-tuple of lists): current board layout, used by generate_moves and make_move
      flags (list of flags): list of flags, used by generate_moves and make_move
      depth (int >=0): depth of the search (number of moves)
    '''
    if depth == 0:
        return chess.lib.heuristics.evaluate(board), [], {}
    ...
    for move in generateMoves(side, board, flags):
        new_side, new_board, new_flags = chess.lib.makeMove(side, board, move[0], move[1], flags, move[2])
        score, next_movelist, next_movetree = alphabeta(new_side, new_board, new_flags, depth - 1, alpha, beta)
